...
% and go to 'videosaur' branch
conda create -n videosaur312 python=3.12 -y
conda activate videosaur312
pip install -r requirements_transition_py312.txt
```

#  2. Timm-based VideoSAUR (Original Implementation)

## Important Note : This works for ~`Commit 1211582` 
...
git clone https://github.com/rbalestr-lab/cjepa.git
% and go to 'videosaur' branch
conda create -n videosaur python=3.8 -y
conda activate videosaur
...
* Download data
```
mkdir -p data/train data/validation data/test
wget http://data.csail.mit.edu/clevrer/videos/train/video_train.zip -O data/train
wget http://data.csail.mit.edu/clevrer/videos/validation/video_validation.zip -O data/validation
wget http://data.csail.mit.edu/clevrer/videos/test/video_test.zip -O data/test
```
...
* Now custom config file : `configs/videosaur/clevrer_dinov2.yml` is the one use should use
