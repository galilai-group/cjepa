...
class CheckValidationCallback(pl.callbacks.Callback):
    ...
    def on_validation_start(self, dummy_trainer, pl_module):
        self.keys = []
        self.num_unique_keys.to(pl_module.device)
        self.num_unique_keys.reset()

    def on_validation_batch_start(self, dummy_trainer, pl_module, batch, batch_idx, dataloader_idx):
        self.keys.extend([key for key in batch["__key__"] if key != "PADDING"])

    def on_validation_end(self, dummy_trainer, pl_module):
        if dummy_trainer.sanity_checking:
