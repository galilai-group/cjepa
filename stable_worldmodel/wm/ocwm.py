...
class OCWM(torch.nn.Module):
    ...
    def encode(
        ...
        # == proprio embeddings
        if (self.proprio_encoder is not None)  and (proprio_key is not None):
        ...
        # == action embeddings
        if (self.action_encoder is not None) and (action_key is not None):
    ...
    def rollout(self, info, action_sequence):
        ...
        info["action_embed"] = (
            init_state_info_dict["action_embed"].unsqueeze(1).repeat_interleave(action_sequence.shape[1], dim=1)
        )  # (B, N, ...)
        info["pixels_embed"] = (
        ...
        if proprio_key is not None:
            info["proprio_embed"] = (
                init_state_info_dict["proprio_embed"]
                .unsqueeze(1)
                .expand(-1, action_sequence.shape[1], *([-1] * (init_state_info_dict["proprio_embed"].ndim - 1)))
            )  # (B, N, ...)
