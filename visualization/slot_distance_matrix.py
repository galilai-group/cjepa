...
def save_video_distance(temporal: list, video_frames: torch.Tensor,
                    output_path: str = "distance_video.mp4", duration: float = 3.5):
    num_frames = len(temporal)
    num_slots = temporal[0].shape[0]
    
    logging.info(f"Creating Distance video with {num_frames} frames, {num_slots} slots")
    ...
    fig = plt.figure(figsize=(12, 6), facecolor='#2a2a2a')
    ...
    # Subplot 2: Temporal distance heatmap
    ax_temporal = fig.add_subplot(132)
    ax_temporal.set_facecolor('#2a2a2a')
    ax_temporal.set_title('Temporal Distance', color='white', fontsize=14, fontweight='bold', pad=10)
    ...
    temporal_im = ax_temporal.imshow(
        temporal_masked[0], cmap='RdYlBu_r', vmin=temporal_min, vmax=temporal_max, aspect='equal'
    )
    ...
    # Create animation
    logging.info("Generating Distance animation...")
    ...
    plt.close(fig)
    logging.info(f"Distance video saved successfully: {output_path}")
...
def slot_interaction(cfg):
    ...
    for t in range(T):
        ...
        for i in range(num_slots):
            for j in range(i+1, num_slots):
                p1 = pred_stacked[t, i, :]  # (d)
                p2 = pred_stacked[t, j, :]  # (d)
                mat[i, j] = np.linalg.norm(p1-p2)
        temporal.append(mat)
    ...
    temporal_upper_valid = temporal_stacked[:, upper_tri_mask]  # exclude first frame
    ...
    logging.info(f"Temporal Distance range after normalization (excl. first frame): [{global_min:.3f}, {global_max:.3f}] -> [0, 1]")
    ...
    output_path = Path(cfg.get('output_dir', './eval_results')) / f"Distance_slot_interaction_{vidname}_{ckpt_name}.mp4"
    output_path.parent.mkdir(parents=True, exist_ok=True)

    save_video_distance(temporal, video_frames_plot, output_path=str(output_path))
