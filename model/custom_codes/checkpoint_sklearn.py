...
class WandbCheckpoint(Callback):
    ...
    def on_load_checkpoint(self, trainer, pl_module, checkpoint):
        ...
        if "wandb" in checkpoint:
            logging.info("Wandb info in checkpoint found, but NOT restoring previous run (using new run instead)... 🔧")
            # Simply remove the wandb info from checkpoint to prevent restoration
            # This allows checkpoint weights to be loaded while starting a fresh wandb run
            del checkpoint["wandb"]
            logging.info("Removed wandb id from checkpoint - will start with new run 🔧")
